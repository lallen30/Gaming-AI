...
@app.route('/gamingai', methods=['POST'])
@require_api_key
def gamingai():
...
def process_youtube_video(video_id):
    """
    Process a YouTube video by extracting its transcript and storing it for training.
    """
    try:
        # Get the transcript
        ...
        # Generate a filename using the video ID
        ...
        return {'status': 'success', 'message': f'YouTube video {video_id} processed successfully'}
    except Exception as e:
        logging.error(f"Error processing YouTube video {video_id}: {e}")
        return {'status': 'error', 'message': str(e)}
...
def process_youtube():
    ...
    result = process_youtube_video(video_id)
    return jsonify(result)
